...
- `idx_slack_metadata_deadline`: deadlineAtカラムにインデックス（期限チェック用）
- `idx_slack_metadata_channel`: channelIdカラムにインデックス
